function [U,n] = refDomFBE2bothPeriodicLINEAR(U, N, n, c, Amp,BP, D, m, kaph, kbet, L, P, H, k, y)
...
while n < N % returns n = N;
    ...
    cen0 = -min(0, v(p1+2:p1+p0-1))./H(p1+1:p1+p0-2) + max(0, v(p1+2:p1+p0-1))./H(p1+2:p1+p0-1) + (H(p1+1:p1+p0-2) + H(p1+2:p1+p0-1))./((xy^2)*(H(p1+2:p1+p0-1).^2).*H(p1+1:p1+p0-2)) - 1;% -min(0, v_j)/h_{j-1} + max(0,v_j)/h_j +(h_{j-1} + h_j)/(xy^2*h_j^2h_{j-1}), j = p1+2,...,p1+p0-1
    ...
    U = A\vertcat(0, U(2:p1-1), 0, 0, U(p1+2:p1+p0-1), 0, 0, U(p1+p0+2:p-1), 0);
