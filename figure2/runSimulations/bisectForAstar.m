...
load('freq1proba5D1m14a80_60lratio15dotPlot7_h03125_e3.mat', 'evalApprox', 'k', 'hstar');
...
% model paramters
BP = 2*[pi,pi]; % BP affects the period. The period of interface i is 2pi/|BP(i)|, i.e. large BP = short period
D = [1, 1];% = [D_1, D_2]
m = [1.4, 1.4]; % = [m_1, m_2]
...
for currC = cVector 
    ...
    for bisect = bisectID
        ...
        toc % done sequence
        if abs(lam(periodIndexBisect-1)-1) < 1e-2 || bisect == bisectID(end)% if we are close enough to 1
